...
window.customElements.define('aabb-concave-sepaxis-canvas', class extends HTMLElement {
	...
	createCanvasElements() {
		...
		this.circle = new Circle(this.shape.topRight, this.shape.bottomEdge.toVector().length() - 1)
	}
	...
	draw() {
		...
		{// shape
			...
			const cpa = p0.add(p1).divide(2)
			const cpb = p2.add(p0).divide(2)
			...
			context.bezierCurveTo(cpa.x, cpa.y, cpb.x, cpb.y, p2.x, p2.y)
			context.closePath()
			context.stroke()
			context.fill()
